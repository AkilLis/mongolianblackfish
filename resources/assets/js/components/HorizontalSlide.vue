	...
	<div :id="slideId" class="Slide__Container" :style="'height:' + slideHeight + 'px'">
		<div :id="horizontalId"  class="Horizontal__Slide">
			<div class="Horizontal__Window" :style="'left:-' + moveLength + widthType +';' + 'width:'+ moveWidth + widthType">
			    <div :style="'position:absolute;left: ' + slideWidth * index + widthType +';' + 'width:' + moveWidth + widthType" class="Horizontal__Content" v-for="(item, index) in items">
			    	<component :item="item" is="trip-slide">
			    		
			    	</component>
			    </div>
	    	</div>
	    	<div v-show="bullet == 1" class="bullet-nav">
	            <div :class="act == index + 1 ? 'active' : ''" @click="setIndex(act - 1)" v-for="act in items.length">
	            </div>
	        </div>
	    </div>
    </div>
...
<script>
	import TripSlide from './context/TripSlide.vue'
	...
	export default {
		...
		created : function () {
			this.slideId = Math.floor(Math.random() * 6) + 1  
			this.horizontalId = Math.floor(Math.random() * 6) + 1  
		},
