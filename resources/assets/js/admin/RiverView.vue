<script>
	import ModifyRiver from './modified/ModifyRiver.vue'
	...
	export default {
		...
		methods : {
			getItems : function () {
				axios.get(config.API_KEY + 'admin/river/all').then(res => {
			...
			onSave: function (data) {
				//$('#loader').modal('show')
				this.saving = true
				axios.post(config.API_KEY + 'admin/river?data=' + data.param, 
					data.formData
				).then(res => {
					if(res.data.code == 0) {
			...
			updateItem : function (item) {
				this.instance = item
				axios.get(config.API_KEY + 'admin/river/' + item.id + '/edit').then(res => {
			...
			deleteItem : function () {
				axios.delete(config.API_KEY + 'admin/river/' + this.selected.id).then(res => {
					if(res.data.code == 0) {
